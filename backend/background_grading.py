...
async def process_grading_job_in_background(
    ...
    try:
        ...
        try:
            ...
            # Don't fail the job here, let finally block handle cleanup/status
            # But add a job-level error to the errors list
            errors.append({
                "filename": "JOB_LEVEL_ERROR",
                "error": f"Job crashed during processing: {str(e)}"
            })

        finally:
            # === JOB FINALIZATION (GUARANTEED TO RUN) ===
            logger.info(f"=== JOB FINALIZATION === {job_id}: {len(submissions)} successful, {len(errors)} errors")
            ...
            # If at least some papers were graded, mark exam as completed
            if len(submissions) > 0:
            ...
            if final_status == "failed":
                message = f"Grading failed for {exam['exam_name']}. Please check the error log."
            elif final_status == "completed_with_errors":
                message = f"Grading completed with errors for {exam['exam_name']}. {len(submissions)} successful, {len(errors)} failed."
            else:
            ...
            await create_notification(
                ...
                title=f"Grading {final_status.replace('_', ' ').title()}",
                message=message,
                link=f"/teacher/review?exam={exam_id}"
            )

    except Exception as e:
        # Catch errors in setup or initial validation (outside the main try block)
        logger.error(f"[Job {job_id}] CRITICAL SETUP ERROR: {e}", exc_info=True)
        await db.grading_jobs.update_one(
            {"job_id": job_id},
            {"$set": {
                "status": "failed",
